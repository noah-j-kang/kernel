...
async def market_maker_loop(bot_id: str, engine: Engine, instrument_id: str = "KERNEL-USD-SPOT"):
    target_price = 10.00
    spread = 0.05
...
async def noise_trader_loop(bot_id: str, engine: Engine, instrument_id: str = "KERNEL-USD-SPOT"):
    engine.update_wallet(bot_id, 100000.0, 0.0)
...
async def options_market_maker_loop(bot_id: str, engine: Engine):
    ...
    while True:
        try:
            spot_price = engine.get_midpoint("KERNEL-USD-SPOT") or 10.00
            time_value = 0.25 
            spread = 0.10
